...
import { CardActionsMenu, CardActionsMenuProps } from "./card-actions-menu";
...
const MetricsCardHeader: React.FC<MetricsCardHeaderProps> = ({
	...
	return (
		<CardHeader
			className={cn(
				"flex flex-col justify-start  !pb-0 items-start gap-4",
				"col-span-3",
				className
			)}>
			{icon}
...
type LayersMetricsCardProps = CoupledMetricsCardProps;

const LayersMetricsCard: React.FC<LayersMetricsCardProps> = ({
	title,
	value,
	actionMenuItems,
	classNames,
	...
	return (
		<MetricsCard className={classNames?.root}>
			<MetricsCardHeader
				...
				icon={
					<Layers className={cn("text-gray-600", classNames?.icon)} />
				}
	...
	return (
		<MetricsCard
			...
			<MetricsCardHeader
				...
				icon={<Layers className={cn("text-white", classNames?.icon)} />}
