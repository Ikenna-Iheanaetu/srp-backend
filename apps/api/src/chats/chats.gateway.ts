        ...
        if (otherParticipantId) {
          ...
          if (otherSocketId) {
            ...
            this.server.to(otherSocketId).emit('message:read', {
              ...
              readBy: userId,
              readAt,
              messageId: latestMessageFromOther?.id || '', // Include actual messageId
            });
    ...
    try {
      ...
      if (chat && result.data.expiresAt) {
        ...
        if (recipientId) {
          const recipientSocketId = await this.cache.getUserSocketId(recipientId);
          if (recipientSocketId) {
            this.server.to(recipientSocketId).emit('chat:accepted', {
              chatId,
    ...
    try {
      ...
      if (chat) {
        ...
        if (recipientId) {
          const recipientSocketId = await this.cache.getUserSocketId(recipientId);
          if (recipientSocketId) {
            this.server.to(recipientSocketId).emit('chat:declined', {
              chatId,
            });
    ...
    try {
      ...
      if (chat && result.data.expiresAt) {
        ...
        if (recipientId) {
          const recipientSocketId = await this.cache.getUserSocketId(recipientId);
          if (recipientSocketId) {
            this.server.to(recipientSocketId).emit('chat:extended', {
              chatId,
              expiresAt: result.data.expiresAt,
            });
    ...
    try {
      ...
      if (chat) {
        ...
        if (recipientId) {
          const recipientSocketId = await this.cache.getUserSocketId(recipientId);
          if (recipientSocketId) {
            this.server.to(recipientSocketId).emit('chat:ended', {
              chatId,
            });
