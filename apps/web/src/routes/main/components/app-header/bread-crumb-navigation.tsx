...
import { Link, LinkProps, Navigate, useLocation } from "react-router";
...
const BreadcrumbNav: React.FC = () => {
	...
	if (!locationCrumbs?.length && pathNameCrumbs.length) {
		...
		return (
			<Navigate
				...
				state={
					{
						crumbs: pathNameCrumbs as Crumbs,
	...
	if (!locationCrumbs || locationCrumbs.length === 0) {
		return null;
	}
	...
	return (
		<Breadcrumb className="bg-gray-50 px-2">
			<BreadcrumbList>
				{locationCrumbs.map(
					({ path, to, label: l, ...linkProps }, index) => {
						const isLastCrumb = index === locationCrumbs.length - 1;
