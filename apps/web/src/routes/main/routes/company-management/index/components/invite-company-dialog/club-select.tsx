/** @format */

import { Button } from "@/components/ui/button";
import { ClubProfileData } from "../../../../profile/club/use-fetch-profile";
import {
	Command,
	CommandEmpty,
	CommandGroup,
	CommandInput,
	CommandItem,
	CommandList,
} from "@/components/ui/command";
import {
	Popover,
	PopoverContent,
	PopoverTrigger,
} from "@/components/ui/popover";
import { clubManagementQueries } from "@/routes/main/routes/club-management/query-factory";
import { useInfiniteQuery } from "@tanstack/react-query";
import React from "react";
import { cn } from "@/lib/utils";
import { Check, ChevronsUpDown } from "lucide-react";
import { CommandLoading } from "cmdk";
import { LoadingIndicator } from "@/components/common/loading-indicator";

export type SelectedClub = Pick<ClubProfileData, "id" | "name" | "refCode">;

export interface ClubSelectProps {
	selectedClub: SelectedClub | null;
	onSelectClub: (club: SelectedClub | null) => void;
	className?: string;
}

export const ClubSelect: React.FC<ClubSelectProps> = ({
	selectedClub,
	onSelectClub,
	className,
}) => {
	const [open, setOpen] = React.useState(false);
	const [search, setSearch] = React.useState("");

	const {
		data,
		hasNextPage,
		fetchNextPage,
		isLoading,
		isFetchingNextPage,
		isFetching,
	} = useInfiniteQuery(clubManagementQueries.infiniteClubs({ search }));

	const fetchedClubs = data?.pages.flatMap((page) => page.data);

	const THRESHOLD_FROM_END = 5;
	const thresholdIndex = fetchedClubs
		? Math.max(0, fetchedClubs.length - THRESHOLD_FROM_END)
		: 0;
	const thresholdRef = (node: HTMLDivElement) => {
		const thresholdObserver = new IntersectionObserver(
			(entries) => {
				const [entry] = entries;
				if (entry?.isIntersecting && hasNextPage && !isFetching) {
					void fetchNextPage();
				}
			},
			{ threshold: 0.1 },
		);

		thresholdObserver.observe(node);

		return () => {
			thresholdObserver.disconnect();
		};
	};

	console.log("fetched clubs", fetchedClubs);

	return (
		<Popover open={open} onOpenChange={setOpen}>
			<PopoverTrigger asChild>
				<Button
					variant="outline"
					role="combobox"
					type="button"
					aria-expanded={open}
					aria-label={selectedClub?.name ?? "Select club"}
					className={cn("w-[200px] justify-between", className)}>
					{selectedClub?.name ?? "Select club..."}
					<ChevronsUpDown />
				</Button>
			</PopoverTrigger>

			<PopoverContent className="w-[200px] p-0" aria-label="Select club">
				<Command
					shouldFilter={
						false // server-side filtering
					}>
					<CommandInput
						placeholder="Search club..."
						className="h-9"
						value={search}
						onValueChange={setSearch}
					/>
					<CommandList className="tw-scrollbar">
						{isLoading ? (
							<CommandLoading className="flex flex-col items-center justify-center gap-1">
								<span>Fetching clubs</span>
								<LoadingIndicator />
							</CommandLoading>
						) : (
							<CommandEmpty>No club found.</CommandEmpty>
						)}

						<CommandGroup>
							{fetchedClubs?.map((club, index) => {
								const isSelected = club.id === selectedClub?.id;

								return (
									<CommandItem
										key={club.id}
										ref={
											index === thresholdIndex
												? thresholdRef
												: undefined
										}
										value={club.name}
										onSelect={() => {
											onSelectClub(
												isSelected ? null : club,
											);
											setOpen(false);
										}}>
										{club.name}
										<Check
											className={cn(
												"ml-auto",
												isSelected
													? "opacity-100"
													: "opacity-0",
											)}
										/>
									</CommandItem>
								);
							})}

							{isFetchingNextPage && (
								<CommandLoading className="my-1">
									<LoadingIndicator />
								</CommandLoading>
							)}
						</CommandGroup>
					</CommandList>
				</Command>
			</PopoverContent>
		</Popover>
	);
};
