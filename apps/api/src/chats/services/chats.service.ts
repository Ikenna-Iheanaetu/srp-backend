...
export class ChatsService {
  ...
  async createChat(userId: string, userType: UserType, dto: CreateChatDto) {
    try {
      ...
      // Note: HIRE_REQUEST_CREATED is logged in createHireRequestAsync
      const initiatorName =
        userId === companyId ? chat.company.name : chat.player.name;
      const recipientName =
        userId === companyId ? chat.player.name : chat.company.name;

      this.logChatEvent(
        ChatEventType.CHAT_INITIATED,
        chat.id,
        undefined,
        `${initiatorName} started a chat with ${recipientName}`,
        undefined,
        {
          initiatorId: userId,
          recipientId,
  ...
  async resendChatRequest(userId: string, dto: { chatId: string }) {
    ...
    try {
      ...
      // Log CHAT_RESENT event (fire and forget)
      const resenderName =
        userId === result.chat.companyId
      ...
      this.logChatEvent(
        ChatEventType.CHAT_RESENT,
        chatId,
        undefined,
        `${resenderName} sent the chat request again`,
        undefined,
        {
          resentBy: userId,
        ...
        this.logger.error(`Failed to log chat resent event: ${error.message}`);
      });
  ...
  async acceptChat(chatId: string, userId: string) {
    try {
      ...
      const acceptorName =
        userId === chat.companyId ? chat.company.name : chat.player.name;

      this.logChatEvent(
        ChatEventType.CHAT_ACCEPTED,
        chatId,
        undefined,
        `${acceptorName} accepted the chat request`,
        undefined,
        {
          acceptedBy: userId,
  ...
  async declineChat(chatId: string, userId: string) {
    try {
      ...
      this.logChatEvent(
        ...
        chatId,
        undefined,
        `${declinerName} declined the chat request`,
        undefined,
        {
          declinedBy: userId,
        },
  ...
  async extendChat(chatId: string, userId: string) {
    try {
      ...
      await this.logChatEvent(
        ...
        chatId,
        undefined,
        `${extenderName} extended the chat for ${daysToAdd} more ${daysToAdd === 1 ? 'day' : 'days'}`,
        extensionDaysEnum,
        {
          extendedBy: userId,
          daysAdded: daysToAdd,
  ...
  async getChatById(chatId: string, userId: string) {
    try {
      ...
      if (chat.status === ChatStatus.PENDING) {
        ...
        // AcceptedChat: expiresAt required, no closedBy
        return {
          message: 'Chat retrieved successfully',
          data: {
            ...baseResponse,
        ...
        // DeclinedChat: closedBy required, no expiresAt
        ...
        // EndedChat: closedBy required, no expiresAt
        const closedBy = chat.closedBy === userId ? 'ME' : 'THEM';
        return {
          message: 'Chat retrieved successfully',
          data: {
        ...
        // ExpiredChat: closedBy is "EXPIRATION", no expiresAt
        return {
          message: 'Chat retrieved successfully',
          data: {
            ...baseResponse,
  ...
  async endChat(chatId: string, userId: string) {
    try {
      ...
      this.logChatEvent(
        ...
        chatId,
        undefined,
        `${enderName} ended the conversation`,
        undefined,
        {
          endedBy: userId,
        },
  ...
  private async updateHireRequestStatusAsync(
    ...
    try {
      ...
      // Fetch the updated HireRequest for logging
      ...
      if (!hireRequest) {
        this.logger.warn(
          `HireRequest not found after update for chat ${chatId}`,
        );
        return;
      }
      ...
      await this.logChatEvent(
        ...
        chatId,
        hireRequest.id,
        `Hire request ${status.toLowerCase().replace('_', ' ')}`,
        undefined,
        {
          requestCode: hireRequest.requestCode,
      ...
      this.logger.debug(
        `HireRequest ${hireRequest.id} updated to ${status} for chat ${chatId}`,
      );
    } catch (error) {
  ...
  private async createHireRequestAsync(
    ...
    try {
      ...
      // Log HIRE_REQUEST_CREATED event
      await this.logChatEvent(
        ChatEventType.HIRE_REQUEST_CREATED,
        chatId,
        hireRequest.id,
        `Hire request created for this conversation`,
        undefined,
        {
          requestCode: hireRequest.requestCode,
          status: hireRequest.status,
