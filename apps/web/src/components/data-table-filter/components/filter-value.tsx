				...
				<div className="inline-flex overflow-x-hidden">
					{hasIcon &&
						(isValidElement(Icon) ? (
							...
							<Icon className="size-4 text-zinc-900 dark:text-zinc-50" />
						))}
				</div>
				<span>{label}</span>
			</div>
...
export function FilterValueDateDisplay<TData>({
	...
	if (filterValues.length === 0) return <Ellipsis className="size-4" />;
	...
	const formattedRangeStr = formatDateRange(startDate, endDate);
...
export function FilterValueNumberDisplay<TData>({
	...
	if (
		...
		const [minValue, maxValueFromFilter] = filter.values;
		const maxValue =
			maxValueFromFilter === Number.POSITIVE_INFINITY ||
			(maxValueFromFilter && maxValueFromFilter >= cappedMax)
				? `${cappedMax}+`
				: maxValueFromFilter;
								...
								<div className="flex items-center gap-1.5">
									...
									{v.icon &&
										(isValidElement(v.icon) ? (
											...
											<v.icon className="size-4 text-zinc-900 dark:text-zinc-50" />
										))}
									<span>
										{v.label}
										<sup
...
export function FilterValueNumberController<TData>({
	...
	const changeMinNumber = (value: number) => {
		const newValues = [value, values[1] ?? 0];
		setValues(newValues);
		actions.setFilterValue(column, newValues);
	};

	const changeMaxNumber = (value: number) => {
		const newValues = [values[0] ?? 0, value];
		setValues(newValues);
		actions.setFilterValue(column, newValues);
	};
	...
	const changeType = useCallback(
		(type: "single" | "range") => {
			const newValues =
				type === "single"
					? [values[0] ?? 0] // Keep the first value for single mode
					: [values[0] ?? 0, values[1] ?? datasetMax ?? 0]; // Use two values for range mode
							...
							<TabsContent
								...
								<div className="flex items-center gap-2">
									...
									<Input
										id="single"
										type="number"
										value={(values[0] ?? 0).toString()} // Use values[0] directly
										onChange={(e) =>
											changeNumber([
