...
>(): FluentColumnConfigHelper<TData> {
	return {
		...
		number: () =>
			new ColumnConfigBuilder<TData, "number", number>("number"),
		date: () => new ColumnConfigBuilder<TData, "date", Date>("date"),
		option: () =>
			new ColumnConfigBuilder<TData, "option", string>("option"),
		multiOption: () =>
			new ColumnConfigBuilder<TData, "multiOption", string[]>(
				"multiOption",
			),
	};
}
	...
	if (column.transformOptionFn) {
		const memoizedTransform = memo(
			...
			(deps) =>
				deps[0]?.map(
					(v) =>
						column.transformOptionFn!(v) as ElementType<
							NonNullable<TVal>
						>,
				),
