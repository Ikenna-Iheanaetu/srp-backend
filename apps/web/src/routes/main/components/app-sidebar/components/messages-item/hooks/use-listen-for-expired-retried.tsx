...
export const useListenForExpiredRetried = () => {
	...
	const onRetried = React.useEffectEvent(
		(chat: EventReceiveResponse<"chat:expired-retried">) => {
			...
			queryClient.setQueryData(queryKey, (old) => {
				...
				const canTransitionToPending = old.status === "EXPIRED";
	...
	React.useEffect(() => {
		chatSocket.on("chat:expired-retried", onRetried);

		return () => {
			chatSocket.off("chat:expired-retried", onRetried);
		};
	}, []);
};
